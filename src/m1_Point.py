...
# Done  yet, just the lines that start the definition of any class:
#
...
class Point(object):
    ...
    def move_to(self, a, b):
        ...
        self.td = self.td + math.sqrt((self.xto - self.x) ** 2 + (self.yto - self.y) ** 2)
    ...
    def move_by(self, dx, dy):
        ...
        self.y = self.y + dy
        self.movement = self.movement + 1
        self.td = self.td + math.sqrt((self.xby - self.x) ** 2 + (self.yby - self.y) ** 2)
    ...
    def get_distance_from_start(self):
        sd = math.sqrt((self.ox - self.x) ** 2 + (self.oy - self.y) ** 2)
        return sd
    ...
    def closer_to(self, a, b):
        s1 = math.sqrt((self.x - a.x) ** 2 + (self.y - a.y) ** 2)
        s2 = math.sqrt((self.x - b.x) ** 2 + (self.y - b.y) ** 2)
        if s1 > s2:
            return b
        if s1 <= s2:
            return a

    def halfway_to(self, b):
        c = Point((self.x + b.x) / 2, (self.y + b.y) / 2)
        return c
